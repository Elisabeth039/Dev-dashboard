...
export default function Folders({
  ...
  const addFolder = () => {
    ...
    setFolders((prev) => {
      const allNotesFolder = prev.find((folder) => folder.id === 1);
      const otherFolders = prev.filter((folder) => folder.id !== 1);

      return allNotesFolder
        ? [allNotesFolder, newFolder, ...otherFolders]
        : [newFolder, ...prev];
    });
    setInput("");
    setIsOpen(false);
      ...
      <div className="folders">
        ...
        {isOpen && (
          <div className="add-folder-pill">
            <button
              className={
                isProject === true ? "add-project-active" : "add-project"
              }
        ...
        <div className="all-folders">
          {folders.map((folder) => (
            <div
              key={folder.id}
              className={`pill ${activeFolder === folder.id ? "active" : ""}`}
            >
              <p
        ...
        {deleteWarning && (
          ...
          >
            <div className="delete-warning">
              <p className="dw-text">
                {" "}
                Are you sure you wanna delete this folder?
              </p>
