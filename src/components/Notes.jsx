...
export default function Notes({
  ...
  const addNote = () => {
    const isEmpty = noteContent.every((block) => block.text.trim() === "");
  ...
  const openNote = (note) => {
    savedScrollTopRef.current = noteListRef.current?.scrollTop || 0;
  ...
  const moveNote = (folderName) => {
    ...
    setNotes((prev) =>
      prev.map((n) =>
        n.id === selectedNote.id ? { ...n, folder: folderName } : n,
      ),
    );
  ...
  const addCheckboxBlock = useCallback(() => {
    const newBlock = {
      id: Date.now(),
      type: "checkbox",
      text: "",
  ...
  const focusBlock = (index) => {
    ...
    setTimeout(() => {
      textareaRef.current[Math.max(index, 0)]?.focus();
    }, 0);
  };
  ...
  useEffect(() => {
    if (!noteOpen && noteListRef.current) {
      noteListRef.current.scrollTop = savedScrollTopRef.current;
    }
  }, [noteOpen]);
  ...
  }, [noteOpen, addCheckboxBlock, addTimeBlock]);
  ...
  const handleEnter = (e, index) => {
    if (e.key === "Enter") {
      ...
      const newBlock = { id: e.timeStamp, type: "text", text: "" };
  ...
  const handleBackspace = (e, block, index) => {
    if (e.key !== "Backspace" || block.text) return;

    if (index === 0 && block.type === "checkbox") {
      e.preventDefault();

      setNoteContent((prev) =>
        prev.map((currentBlock, currentIndex) =>
          currentIndex === 0
            ? { id: currentBlock.id, type: "text", text: "" }
            : currentBlock,
        ),
      );
  ...
  const isCompleted = (noteContentArray) => {
    const checkboxes = noteContentArray.filter(
  ...
  return (
    <div className="notes-section">
      {noteOpen ? (
        <div className="note">
          <div className="functions-panel">
            <button className="function" onClick={addCheckboxBlock} title="F1">
              ☑
            </button>
            <button className="function" onClick={addTimeBlock} title="F2">
              T
            </button>
          ...
          <div className="note-area">
            <div className="title">
              <button onClick={addNote} className="close-note">
                {"<"}
              </button>
              ...
              {menuOpen && (
                <div className="menu">
                  <button
                    ...
                    onClick={() => {
                      if (selectedNote) {
                        pinNotes(selectedNote.id);
                      }
                      setMenuOpen(false);
                    }}
                  >
                    {selectedNote.pinned === true ? "Unpin note" : "Pin note"}
                  </button>
                  <button
                    className="menu-btn"
                    onClick={() => {
        ...
        <>
          <div className="note-list" ref={noteListRef}>
            {notes
              .filter((note) =>
                activeFolder === 1 ? true : note.folder === activeFolder,
              )
              .filter((note) => {
                const noteText = (
                  note.title +
          ...
          <div
            ...
            onClick={() => {
              setNoteContent([{ id: Date.now(), type: "text", text: "" }]);
              setSelectedNote(null);
              setNoteTitle("");
              setNoteOpen(true);
            }}
      ...
      {deleteWarning && (
        <div
          className={`backdrop ${deleteWarning ? "show" : ""}`}
